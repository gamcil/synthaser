...
def test_CDSearch_new_search_bad_response(monkeypatch):
    query_ids = ["test"]

    def response(ids):
        return SynthaseContainer([Synthase(header="test")])

    monkeypatch.setattr(ncbi, "_container_from_query_ids", response)
...
def test_container_from_query_file(tmp_path):
    ...
    with fasta.open() as fp:
        sc = ncbi._container_from_query_file(fp)

    assert sc == SynthaseContainer([Synthase(header="one", sequence="abcdef")])


def test_container_from_query_ids_valueerror():
    with pytest.raises(ValueError):
        ncbi._container_from_query_ids(1)
...
def test_container_from_query_ids_iterable():
    ...
    with requests_mock.Mocker() as m:
        ...
        sc = ncbi._container_from_query_ids(ids)
...
def test_container_from_query_ids_file(tmp_path, monkeypatch):
    ...
    sc = ncbi._container_from_query_ids([ids.name])
...
def test_prepare_input_too_many_sequences(monkeypatch):
    def response(ids):
        return SynthaseContainer([Synthase()] * 4001)

    monkeypatch.setattr(ncbi, "_container_from_query_ids", response)

    with pytest.raises(ValueError):
        ncbi.prepare_input(query_ids=["test"])
